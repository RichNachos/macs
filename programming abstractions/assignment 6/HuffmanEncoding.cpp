...
Node* buildEncodingTree(Map<ext_char, int>& frequencies) {
	...
	foreach (ext_char c in frequencies) {
		...
		sNode->character = c;
		sNode->weight = frequencies[c];
		sNode->zero = NULL;
		sNode->one = NULL;

		pqueue.enqueue(sNode, sNode->weight);
	}
	...
	while (pqueue.size() != 1) {
		if (pqueue.size() >= 2) {
			...
			parent->zero = zero;
			parent->one = one;
			pqueue.enqueue(parent, parent->weight);
		}
	}
...
void encodeFile(istream& infile, Node* encodingTree, obstream& outfile) {
	ext_char c;
	Map<ext_char,string> map = getCodeMap(encodingTree);
...
// Returns code map
Map<ext_char, string> getCodeMap(Node* encodingTree) {
	Map<ext_char, string> map;
	buildEncodingMap(map, encodingTree, "");
	return map;
}

// Builds a map which has key ext_char and value corresponding to its encoded binary string
void buildEncodingMap(Map<ext_char, string>& map, Node* encodingTree, string code) {
	if (encodingTree == NULL) {
		return;
	}
	if (encodingTree->character != NOT_A_CHAR) {
		map[encodingTree->character] = code;
		return;
	}
	buildEncodingMap(map, encodingTree->zero, code + "0");
	buildEncodingMap(map, encodingTree->one, code + "1");
}
