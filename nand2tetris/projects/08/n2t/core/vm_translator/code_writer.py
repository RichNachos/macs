...
class CodeWriter:
    ...
    def write_label(self, segment: str) -> None:
        self.write([f"({segment})"])

    def write_goto(self, segment: str) -> None:
        self.write([f"@{segment}", "0;JMP"])

    def write_if(self, segment: str) -> None:
        self.write(Shortcuts.POP_SUFFIX[:3] + [f"@{segment}", "D;JNE"])

    def write_function(self, function_name: str, nvars: int) -> None:
        asm = [f"({function_name})"]
        local = ["@0", "D=A"] + Shortcuts.PUSH_SUFFIX
        local = local * nvars
        asm += local
        self.write(asm)
    ...
    def write_call(self, function_name: str, nargs: int) -> None:
        self.label_index += 1
        return_address = [f"@{function_name}.return.{self.label_index}"]
        pointer = [f"@{nargs}", "D=D-A", "@ARG", "M=D", "@SP", "D=M", "@LCL", "M=D"]
        pointer += [
            f"@{function_name}",
            "0;JMP",
            f"({function_name}.return.{self.label_index})",
        ]
        asm = return_address + Shortcuts.CALL + pointer
        self.write(asm)

    def write_return(self) -> None:
        asm = (
            ["@LCL", "D=M", "@R13", "M=D", "@5", "A=D-A", "D=M", "@R14", "M=D"]
            + Shortcuts.POP_SUFFIX[:3]
            + [
                "@ARG",
                "A=M",
                "M=D",
