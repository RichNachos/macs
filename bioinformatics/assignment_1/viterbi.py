    ...
    for i in xrange(0,L):
        ...
        for k in xrange(N):
            if i == 0:
                ...
                max_p = -10e10
                max_s = None
                for prev_s in xrange(N):
                    p = Vprev[prev_s] + log(tr[prev_s][k]) + log(em[k][X[i]])
                    if p > max_p:
                        max_p = p
                        max_s = prev_s
                V[i][k] = max_p
...
def print_annostats(X,anno,filename):
    ...
    print "Saving High-GC length histogram to %s_highgc.ps" % filename
    p = plothist(lengths[0],low=0)
    p.save(filename+"_highgc.ps")
    print "Saving Low-GC length histogram to %s_lowgc.ps" % filename
    p = plothist(lengths[1],low=0)
    p.save(filename+"_lowgc.ps")
