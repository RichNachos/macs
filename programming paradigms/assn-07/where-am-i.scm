...
(define (partition pivot num-list comparator)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) comparator)))
	(if (comparator (car num-list) pivot)
...
(define (quicksort num-list comparator)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) comparator)))
	(append (quicksort (car split) comparator) 
		(list (car num-list)) 
		(quicksort (cadr split) comparator)))))
...
(define (intersection-points circle-list)
  (if (or (null? (cdr circle-list))
          (null? circle-list)) '()
          (append (apply append (map (lambda (circle) (intersect (car circle-list) circle))
                                (cdr circle-list)))
                  (intersection-points (cdr circle-list)))))

;; 2
(define (distance-product point point-list)
        (apply * (map (lambda (x) (dist point x))
                      (remove point point-list))))

;; 3
(define (rate-points point-list)
        (map (lambda (x) (list (distance-product x point-list) x)) point-list))

;; 4
(define (sort-points rated-point-list)
        (quicksort rated-point-list (lambda (x y) (< (car x) (car y)))))

;; 5
(define (clumped-points point-list)
        (apply append (map cdr (prefix-of-list (sort-points (rate-points point-list))
                                 (quotient (length point-list) 2)))))

;; 6
(define (average-point point-list)
        (let ((ax (/ (apply + (map x point-list)) (length point-list)))
	            (ay (/ (apply + (map y point-list)) (length point-list))))
             (list (distance-product (make-pt ax ay) point-list) (make-pt ax ay))))
...
;;8
(define (where-am-i distance-list star-list)
        (sort-points (map best-estimate (all-guesses distance-list star-list))))
