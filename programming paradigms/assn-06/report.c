...
{
  sem_wait(&(bank->lockReportTransfer));
  ...
  if (amountAbs < reportingAmount) {
    sem_post(&(bank->lockReportTransfer));
...
{
  sem_wait(&(bank->reportCheck));
  bank->unfinishedWorkers--;
  if (bank->unfinishedWorkers != 0) {
    sem_post(&(bank->reportCheck));
    sem_wait(&(bank->startDay));
    return 0;
  }
  sem_post(&(bank->reportCheck));
  Report *rpt = bank->report;
